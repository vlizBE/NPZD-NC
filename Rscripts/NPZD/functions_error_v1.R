...
convertZoo <- function(zoo_obs, num_station){
  ...
  noctiluca <- zoo_obs[which(zoo_obs$Taxon2 ==  taxa_select[2]),]
  ...
  ##############Harpacticoida
  harpa <- zoo_obs[which(zoo_obs$Taxon2 ==  taxa_select[3]),]
  ...
  ##############Appendicularia
  append <- zoo_obs[which(zoo_obs$Taxon2 ==  taxa_select[4]),]
...
calculateErrors <- function(num_station, chla, zoo_aggregated, num_sim){
  ...
  cont <- 1
  possible_parameters <- read.csv(paste0("~/workspace/NPZ/Output/Intermediate results/station", num_station, "_iter1/possible_parameters_iter1.csv"))
  ...
  for(i in 1:num_sim){
    ...
    output <- read.csv(paste0("~/workspace/NPZ/Output/Intermediate results/station", num_station, "_iter1/detailed_simulation_", i, ".csv"))
    ...
    if(!anyNA(output)){
      ...
      if(num_station == "offshore"){
        temp_error<-calculateRMSE(output, possible_parameters[i,12],i, zoo_aggregated, chla, num_station)
        
      }else{
        temp_error<-calculateRMSE(output, possible_parameters[i,12],i, zoo_aggregated[,c(1,2)], chla[,c(2,3)], num_station)
      ...
      errors_parameters[cont,c(5:18)] <-possible_parameters[i,]
  ...
  write.csv(errors_parameters, paste0("~/workspace/NPZ/Output/Intermediate results/station", num_station,"_iter1/errors_parameters_iter1.csv"))
...
createErrorFile <- function(numSim){
  errors_parameters <- data.frame(matrix(nrow = numSim, ncol = 18))
  ...
  names(errors_parameters)[5] <- "ID"
  names(errors_parameters)[6] <- "maxUptake"
  names(errors_parameters)[7] <- "ksPAR"
  names(errors_parameters)[8] <- "ksDIN"
  names(errors_parameters)[9] <- "ksP"
  names(errors_parameters)[10] <- "ksSi"
  names(errors_parameters)[11] <- "maxGrazing"
  names(errors_parameters)[12] <- "ksGrazing"
  names(errors_parameters)[13] <- "pFaeces"
  names(errors_parameters)[14] <- "excretionRate"
  names(errors_parameters)[15] <- "mortalityRate"
  names(errors_parameters)[16] <- "ChlNratio"
  names(errors_parameters)[17] <- "Tobs"
  names(errors_parameters)[18] <- "kd"
  return(errors_parameters)
}

calculateRMSE <- function(resultsSimulation1, chla_conversion, id_temp, zoo_obs, chla_obs, numStation){
  ...
  rmse_sim_phyto <- rmse_sim_chla/chla_conversion
...
createBestSim <- function(ID_minError, station_int, chla_obs, zoo_obs){
  ...
  for(i in 1:countID){
    idSim <- ID_minError[i]
    print(idSim)
    resultsSim <-read.csv(paste0("~/workspace/NPZ/Output/Intermediate results/station", station_int, "_iter1/detailed_simulation_", idSim, ".csv"))
