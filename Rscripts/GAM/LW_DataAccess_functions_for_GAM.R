...
get_abiotic_pigment <- function(start, stop, stationselection, stationregion){
...
################## ABIOTIC ##################
  ...
  df_all <- df_all %>%
    mutate(
      areas = plyr::mapvalues(
        station, 
        from = station_selection,
        to = station_region
      ),
...
################## PIGMENTS ##################
  ...
  write.csv(abiotic_pigment_2020, file = paste0("~/stevenp/BlueCloud/Data/test/abiotic_pigment_2020.csv"),row.names = F)

return(df_GAM)
}

#########################################################################################################################
get_zooplankton <- function(start, stop, taxa_select, stationselection){
  ...
  # change date format
  df_zoo$time <- strftime(df_zoo$time, format = "%m/%d/%Y")
  ...
  colnames(df_zoo) <- c("", "X", "Station",	"Longitude",	"Latitude",	"Tripaction",	"Date",
                         "Taxon",	"Density",	"Month",	"Year",	"Source")
  
  
  # write .csv file for NPZ model
  write.csv(dfZoo, file = paste0("~/stevenp/BlueCloud/Data/test/zooscan.csv"), row.names = F)
