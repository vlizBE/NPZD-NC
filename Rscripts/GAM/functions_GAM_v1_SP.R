...
test_model <-function(y_values, day_130, year_130, k_day, k_year){
  
  model_1 <- gam(y_values ~ s(day_130, k = k_day) + s(year_130, k = k_year), method = "REML")
...
test_model_gamma <-function(y_values, day_130, year_130, k_day, k_year, g1){
  
  model_1 <- gam(y_values ~ s(day_130, k = k_day) + s(year_130, k = k_year), method = "REML", gamma = g1)
...
comparisonPoint <- function(day_130, year_130, y_value, model_base) {
  
  compare<- data.frame(day_130, year_130, y_value)
  
  names(compare)[1] <- "day_130"
  names(compare)[2] <- "year_130"
